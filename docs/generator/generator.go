...
func saveIndex(links []string, out string) {
	markerStart := "\n// START: Autogenerated catalog (DO NOT EDIT)\n"
	markerEnd := "// END: Autogenerated catalog (DO NOT EDIT)\n"
	...
	dest := filepath.Join(out, "pages", "index.adoc")
	exBytes, err := ioutil.ReadFile(dest)
	if err != nil {
		handleGeneralError("could not read index file", err)
	}
	existing := string(exBytes)
	posStart := strings.Index(existing, markerStart)
	posEnd := strings.Index(existing, markerEnd) + len(markerEnd)
	if posStart < 0 {
		posStart = len(existing)
	}
	if posEnd < posStart {
		posEnd = posStart
	}
	if posEnd > len(existing) {
		posEnd = len(existing)
	}
	head := existing[0:posStart]
	tail := existing[posEnd:]
	newContent := head + content + tail
	if _, err := os.Stat(dest); err == nil {
		err = os.Remove(dest)
		handleGeneralError(fmt.Sprintf("cannot remove file %q", dest), err)
	}
	err = ioutil.WriteFile(dest, []byte(newContent), 0666)
	handleGeneralError(fmt.Sprintf("cannot write file %q", dest), err)
	fmt.Printf("%q written\n", dest)
}
